 ...
 */
export async function createCoupon(data: Partial<Coupon>): Promise<{ success: boolean; error?: string; coupon?: Coupon }> {
 ...
 */
export async function toggleCouponActive(id: string, is_active: boolean): Promise<{ success: boolean; error?: string }> {
    const { error } = await supabase
        .from('coupons')
        .update({ is_active })
        .eq('id', id)
...
/**
 * Get redemption records for a coupon.
 */
