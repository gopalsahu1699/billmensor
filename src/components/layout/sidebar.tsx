...
import { MdDashboard, MdPeople, MdInventory, MdShoppingCart, MdCreditCard, MdBarChart, MdSettings, MdExpandMore, MdWallet, MdClose, MdLogout, MdAdd, MdChevronLeft, MdChevronRight, MdDescription, MdHistory } from "react-icons/md";
        ...
        <>
            ...
            >
                <div className="p-6 flex items-center justify-between border-b border-white/5 bg-slate-900 dark:bg-slate-950">
                    ...
                    <button onClick={closeMobileMenu} className="lg:hidden p-2 text-slate-400 hover:bg-white/5 rounded-xl">
                        <MdClose size={20} />
                    </button>
                </div>
                ...
                <div className="p-4 px-3">
                    <Link
                        ...
                        onClick={() => { if (showMobileMenu) closeMobileMenu(); }}
                        className={cn(
                ...
                <nav className="flex-1 p-3 pt-2 space-y-1 overflow-y-auto scrollbar-hide">
                    <div className="space-y-1">
                        <SidebarLink href="/dashboard" icon={<MdDashboard size={18} />} label={t("Dashboard")} active={isActive("/dashboard")} onClick={closeMobileMenu} isCollapsed={isCollapsed && !showMobileMenu} />
                        <SidebarLink href="/dashboard/customers" icon={<MdPeople size={18} />} label={t("Customers")} active={isActive("/dashboard/customers")} onClick={closeMobileMenu} isCollapsed={isCollapsed && !showMobileMenu} />
                        <SidebarLink href="/dashboard/day-book" icon={<MdDescription size={18} />} label="Day Book" active={isActive("/dashboard/day-book")} onClick={closeMobileMenu} isCollapsed={isCollapsed && !showMobileMenu} />
                        <SidebarLink href="/dashboard/returns" icon={<MdHistory size={18} />} label="Returns" active={isActive("/dashboard/returns")} onClick={closeMobileMenu} isCollapsed={isCollapsed && !showMobileMenu} />
                    </div>

                    <Dropdown icon={<MdInventory size={18} />} label={t("Inventory")} open={inventoryOpen} setOpen={setInventoryOpen} active={isActive("/dashboard/products") || isActive("/dashboard/alerts")} isCollapsed={isCollapsed && !showMobileMenu} setIsCollapsed={setIsCollapsed}>
                        <SubLink href="/dashboard/products" label="Items / Products" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/alerts" label="Stock Alerts" onClick={closeMobileMenu} />
                    </Dropdown>

                    <Dropdown icon={<MdShoppingCart size={18} />} label={t("Sales")} open={salesOpen} setOpen={setSalesOpen} active={pathname.includes("invoices") || pathname.includes("quotations") || pathname.startsWith("/dashboard/orders/sales")} isCollapsed={isCollapsed && !showMobileMenu} setIsCollapsed={setIsCollapsed}>
                        ...
                        <SubLink href="/dashboard/orders/sales" label="Sales Orders" onClick={closeMobileMenu} />
                    ...
                    <Dropdown icon={<MdCreditCard size={18} />} label={t("Purchase")} open={purchaseOpen} setOpen={setPurchaseOpen} active={isActive("/dashboard/purchases") || pathname.startsWith("/dashboard/orders/purchase")} isCollapsed={isCollapsed && !showMobileMenu} setIsCollapsed={setIsCollapsed}>
                        <SubLink href="/dashboard/purchases" label="Purchase Bills" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/orders/purchase" label="Purchase Orders" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/payments-out" label="Payment Out" onClick={closeMobileMenu} />
                    </Dropdown>

                    <Dropdown icon={<MdWallet size={18} />} label={t("Finance")} open={financeOpen} setOpen={setFinanceOpen} active={isActive("/dashboard/expenses") || isActive("/dashboard/cash-flow") || isActive("/dashboard/cheques")} isCollapsed={isCollapsed && !showMobileMenu} setIsCollapsed={setIsCollapsed}>
                        <SubLink href="/dashboard/expenses" label="Expenses" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/cash-flow" label="Cash Flow" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/cheques" label="Cheques" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/settings/bank-accounts" label="Bank Accounts" onClick={closeMobileMenu} />
                    </Dropdown>

                    <Dropdown icon={<FiBox size={18} />} label={t("Smart Tools")} open={toolsOpen} setOpen={setToolsOpen} active={isActive("/dashboard/scanner") || isActive("/dashboard/store") || isActive("/dashboard/reminders") || isActive("/dashboard/pos")} isCollapsed={isCollapsed && !showMobileMenu} setIsCollapsed={setIsCollapsed}>
                        <SubLink href="/dashboard/scanner" label="OCR Scanner" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/store" label="Online Store" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/reminders" label="Payment Reminders" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/pos" label="POS Terminal" onClick={closeMobileMenu} />
                        <SubLink href="/dashboard/staff" label="Staff & Payroll" onClick={closeMobileMenu} />
                    </Dropdown>

                    <SidebarLink href="/dashboard/reports" icon={<MdBarChart size={18} />} label={t("Reports")} active={isActive("/dashboard/reports")} onClick={closeMobileMenu} isCollapsed={isCollapsed && !showMobileMenu} />

                    <div className="mt-6 pt-6 border-t border-white/5">
                        <Dropdown icon={<MdSettings size={18} />} label={t("Settings")} open={settingsOpen} setOpen={setSettingsOpen} active={pathname.startsWith("/dashboard/settings")} isCollapsed={isCollapsed && !showMobileMenu} setIsCollapsed={setIsCollapsed}>
                            <SubLink href="/dashboard/settings/account" label="Account Setting" onClick={closeMobileMenu} />
                            <SubLink href="/dashboard/settings/team" label="Team Members" onClick={closeMobileMenu} />
                            <SubLink href="/dashboard/settings/print" label="Print Setting" onClick={closeMobileMenu} />
                            <SubLink href="/dashboard/settings/e-invoice" label="E-Invoice" onClick={closeMobileMenu} />
                            <SubLink href="/dashboard/settings/backup" label="Backup & Restore" onClick={closeMobileMenu} />
...
function SidebarLink({ href, icon, label, active, onClick, isCollapsed }: SidebarLinkProps) {
    return (
        <Link
            href={href}
        ...
        >
            <div className="shrink-0 w-5 h-5 flex items-center font-bold">{icon}</div>
...
function Dropdown({ icon, label, open, setOpen, active, children, isCollapsed, setIsCollapsed }: DropdownProps) {
    if (isCollapsed) {
        return (
            <button
                onClick={() => { if (setIsCollapsed) setIsCollapsed(false); setOpen(true); }}
                className={cn(
    ...
    return (
        <div className="space-y-1">
            ...
            >
                ...
                <MdExpandMore size={16} className={cn("transition-transform duration-200", open && "rotate-180")} />
            </button>
            <div className={cn(
