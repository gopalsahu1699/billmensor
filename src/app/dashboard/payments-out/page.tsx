...
import { Plus, Search, ArrowUpRight, Wallet, Banknote, CreditCard } from 'lucide-react'
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight italic uppercase">Payment Out</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium mt-2 leading-relaxed">Record payments made to suppliers. Manage your cash outflows efficiently.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 dark:bg-slate-800/30">
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5">Voucher Details</th>
                                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5">Supplier</th>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-white/5 font-medium">
                            {filteredPayments.map((pmt) => (
                                <tr
                                    ...
                                    onClick={() => router.push(`/dashboard/payments-out/${pmt.id}`)}
                                >
                                    <td className="px-8 py-6">
                                        <div>
                                            <p className="text-sm font-black text-red-600 dark:text-red-400 leading-tight group-hover:underline">{pmt.payment_number}</p>
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-xs font-black text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/10 group-hover:bg-red-600/10 group-hover:text-red-500 group-hover:border-red-600/20 transition-all">
                                                {pmt.customers?.name?.charAt(0).toUpperCase()}
                                            </div>
                                            <span className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-red-500 transition-colors">{pmt.customers?.name}</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
