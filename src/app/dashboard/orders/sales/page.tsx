...
import { SalesOrder } from '@/types/index'
...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
    ...
    shipped: { label: 'Shipped', color: 'bg-indigo-100 text-indigo-700', icon: FaTruck },
    delivered: { label: 'Delivered', color: 'bg-green-100 text-green-700', icon: IoCheckmarkCircle },
    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white">Sales Orders</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your sales orders and track deliveries</p>
                </div>
                <Link href="/dashboard/orders/sales/create">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                {filteredOrders.length === 0 ? (
                    <div className="py-20 text-center">
                        ...
                        <p className="text-slate-500 mt-1">Create your first sales order to get started</p>
                    </div>
                ) : (
                    <table className="w-full">
                        <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                            <tr>
                                ...
                                <th className="text-left px-6 py-4 text-xs font-black text-slate-500 uppercase tracking-widest">Customer</th>
                            ...
                            {filteredOrders.map((order) => {
                                ...
                                return (
                                    <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-slate-600 dark:text-slate-300">{order.customers?.name || 'N/A'}</span>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link href={`/dashboard/orders/sales/${order.id}`}>
                                                    <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">
                                                        <IoEye size={18} className="text-slate-400" />
                                                    </button>
                                                </Link>
                                                <Link href={`/dashboard/orders/sales/${order.id}?edit=true`}>
                                                    <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">
