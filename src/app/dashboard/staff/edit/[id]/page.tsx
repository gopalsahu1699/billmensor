    ...
    useEffect(() => {
        const fetchStaff = async () => {
            try {
                ...
                console.error(error)
                toast.error(friendlyError(error, 'Failed to load staff details'))
            } finally {
                setLoading(false)
            }
        }
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            const { error } = await supabase.from('staff_members').update({
                ...
                photo_url: imagePreview?.startsWith('data:') ? formData.photo_url : imagePreview
            }).eq('id', staffId)
            ...
            console.error(error)
            toast.error(friendlyError(error, 'Failed to update staff member. Ensure database columns exist.'))
        } finally {
            setSaving(false)
        }
    }
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-slate-50 dark:bg-[#020617]">
                <MdRefresh className="animate-spin text-violet-600 w-12 h-12" aria-hidden="true" />
                ...
                <div className="lg:col-span-1 space-y-8">
                    ...
                    >
                        <div className="flex flex-col items-center text-center">
                            <label className="relative group cursor-pointer mb-6" aria-label="Change profile photo">
