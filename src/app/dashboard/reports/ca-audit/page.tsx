...
export default function CAAuditReportPage() {
  ...
  const fetchAuditData = useCallback(async () => {
    try {
      ...
      const salesAgg = (invoices || []).reduce(
        (acc, inv) => {
      ...
      const expAgg = (expenses || []).reduce(
        (acc, exp) => {
          const cat = exp.category || "General";
          acc.total += Number(exp.amount || 0);
          acc.categories[cat] =
            (acc.categories[cat] || 0) + Number(exp.amount || 0);
          acc.list.push(exp);
          return acc;
        },
        {
      ...
      const retAgg = (returnsData || []).reduce(
        (acc, ret) => {
      ...
      setAuditData({
        ...
        netProfit:
          salesAgg.taxable -
          retAgg.salesTaxable -
          (purAgg.taxable - retAgg.purchaseTaxable) -
          expAgg.total,
      });
    } catch (error: unknown) {
      const msg =
        error instanceof Error
          ? error.message
          : "Failed to aggregate audit data";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleShare = async () => {
    await sharePDF({
      ...
      filename: `CA_Audit_Report_${dateRange.start}.pdf`,
      title: "Consolidated Audit Report",
      text: `Attached is the consolidated audit report for the period ${dateRange.start} to ${dateRange.end}.`
    });
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 no-print">
        <div className="flex items-center gap-4">
          <Button
            ...
            size="sm"
            onClick={() => router.back()}
            className="rounded-2xl h-12 w-12 hover:bg-slate-100 transition-all"
          >
      ...
      <div
        id="ca-audit-render"
        className="bg-white dark:bg-slate-900 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-2xl overflow-hidden print:shadow-none print:border-none"
      >
        <div className="p-12 lg:p-20 space-y-16">
          ...
          <div className="flex justify-between items-start border-b-2 border-slate-900 pb-12">
            ...
            <div className="text-right space-y-6">
              <div>
                ...
                <p className="text-[11px] font-black text-primary mt-1 uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-blue-50/50 dark:bg-blue-900/10 p-8 rounded-4xl border border-blue-100/50 dark:border-blue-800/50">
              <p className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest mb-4">
                Total Revenue (Gross)
              </p>
              <h3 className="text-3xl font-black text-slate-900 dark:text-white italic">
                ₹{auditData.sales.total.toLocaleString("en-IN")}
              </h3>
              <div className="mt-4 pt-4 border-t border-blue-200/30 dark:border-blue-800/30 flex justify-between text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter">
                ...
                <span>
                  Taxable: ₹{auditData.sales.taxable.toLocaleString("en-IN")}
                </span>
              </div>
            </div>
            <div className="bg-amber-50/50 dark:bg-amber-900/10 p-8 rounded-4xl border border-amber-100/50 dark:border-amber-800/50">
              ...
              <h3 className="text-3xl font-black text-slate-900 dark:text-white italic">
                ₹{auditData.purchases.total.toLocaleString("en-IN")}
              </h3>
              <div className="mt-4 pt-4 border-t border-amber-200/30 dark:border-amber-800/30 flex justify-between text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-tighter">
                ...
                <span>
                  Input Credit: ₹
                  {auditData.purchases.tax.toLocaleString("en-IN")}
                </span>
              </div>
            </div>
            <div className="bg-slate-900 dark:bg-blue-900/30 p-8 rounded-4xl text-white shadow-xl shadow-slate-900/20 border border-slate-800 dark:border-blue-800/50">
              ...
              <h3
                className={`text-4xl font-black italic ${auditData.netProfit >= 0 ? "text-green-400" : "text-red-400"}`}
              >
                ₹{Math.abs(auditData.netProfit).toLocaleString("en-IN")}
                {auditData.netProfit < 0 && (
                  <span className="text-sm ml-2 font-black uppercase tracking-widest">
                    (Loss)
                  </span>
                )}
              </h3>
              <p className="text-[10px] text-slate-500 dark:text-slate-500 mt-2 font-bold uppercase tracking-widest">
          ...
          <div className="space-y-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  <tr>
                    <td className="px-6 py-6 font-black text-slate-900 dark:text-slate-100 uppercase italic">
                      IGST (Inter-state)
                    </td>
                    <td className="px-6 py-6 text-right font-bold text-slate-500 dark:text-slate-400">
                      ₹{auditData.sales.igstTaxable.toLocaleString("en-IN")}
                    </td>
                    <td className="px-6 py-6 text-right font-black text-blue-600 dark:text-blue-400">
                      ₹{auditData.sales.igst.toLocaleString("en-IN")}
                    </td>
                    <td className="px-6 py-6 text-right font-black text-slate-900 dark:text-slate-100">
                      ₹
                      {(
                        auditData.sales.igstTaxable + auditData.sales.igst
                  ...
                  <tr>
                    ...
                    <td className="px-6 py-6 text-right font-bold text-slate-500 dark:text-slate-400">
                      ₹{auditData.sales.localTaxable.toLocaleString("en-IN")}
                    </td>
                    <td className="px-6 py-6 text-right font-black text-blue-600 dark:text-blue-400">
                      ₹{auditData.sales.cgst.toLocaleString("en-IN")}
                    </td>
                    <td className="px-6 py-6 text-right font-black text-slate-900 dark:text-slate-100">
                      ₹
                      {(
                        auditData.sales.localTaxable + auditData.sales.cgst
                      ).toLocaleString("en-IN")}
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td className="px-6 py-6 text-right font-black text-blue-600 dark:text-blue-400">
                      ₹{auditData.sales.sgst.toLocaleString("en-IN")}
                    </td>
                    <td className="px-6 py-6 text-right font-black text-slate-900 dark:text-slate-100">
                      ₹
                      {(
                        auditData.sales.localTaxable + auditData.sales.sgst
                      ).toLocaleString("en-IN")}
                    </td>
                  </tr>
                </tbody>
            ...
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-3 px-2 italic">
              ⓘ Note: CGST &amp; SGST rows reflect the{" "}
              <span className="text-slate-600 font-black">
                same intra-state transactions
              </span>{" "}
              — each is half the total GST. Do not add their taxable amounts
              together. Total intra-state gross = ₹
              {(
                auditData.sales.localTaxable +
                auditData.sales.cgst +
                auditData.sales.sgst
              ).toLocaleString("en-IN")}
              .
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {Object.entries(auditData.expenses.categories).map(
                  ([cat, amt]) => (
                    <div
                      key={cat}
                      className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800"
                    >
                ...
                <div className="pt-4 flex justify-between items-center border-t-2 border-slate-900 dark:border-slate-700">
                  <span className="text-[11px] font-black uppercase text-slate-900 dark:text-slate-100 tracking-widest">
                    Total Operating Cost
                  </span>
                  <span className="text-lg font-black text-slate-900 dark:text-slate-100">
                    ₹{auditData.expenses.total.toLocaleString("en-IN")}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900 rounded-4xl p-10 flex flex-col justify-between text-white border-4 border-slate-800">
              <div>
                <h4 className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-4">
                  Verification Note
                </h4>
                <p className="text-xs text-slate-400 leading-relaxed italic">
                  &ldquo;This report summarizes all digital records for the
                  selected period. For accurate tax assessment, please verify
                  these figures against your bank statements and physical
                  receipts.&rdquo;
                </p>
              </div>
              <div className="pt-10 space-y-4">
                ...
                <div className="flex justify-between text-[11px] font-black uppercase tracking-widest">
                  ...
                  <span className="text-xl italic">
                    ...
                    {Math.max(
                      0,
                      auditData.sales.igst +
                      auditData.sales.cgst +
                      auditData.sales.sgst -
                      auditData.purchases.tax,
                    ).toLocaleString("en-IN")}
                  </span>
                </div>
                {auditData.purchases.tax >
          ...
          <div className="space-y-16 pt-16 border-t-2 border-slate-900">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight italic">
                  Annexure: Detailed Transaction Logs
                </h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Complete list of all recorded transactions for
                  cross-verification
                </p>
              </div>
            </div>
            ...
            {/* Sales Register */}
            <div className="space-y-6">
              <h4 className="text-xs font-black text-slate-900 uppercase tracking-[0.3em] pl-2 border-l-4 border-blue-600">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-[10px]">
                  <thead className="bg-slate-50 dark:bg-slate-800/50 font-black uppercase tracking-widest text-slate-400 border-y border-slate-100 dark:border-slate-800">
                    <tr>
                      <th className="px-4 py-3 text-left font-black tracking-widest">
                        Date
                      </th>
                      <th className="px-4 py-3 text-left font-black tracking-widest">
                        Invoice #
                      ...
                      <th className="px-4 py-3 text-right font-black tracking-widest">
                        Taxable
                      </th>
                      <th className="px-4 py-3 text-right font-black tracking-widest">
                        GST
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800 font-bold text-slate-600 dark:text-slate-400">
                    ...
                    {auditData.sales.list.map((inv: any) => (
                      <tr key={inv.id}>
                        ...
                        <td className="px-4 py-3 text-slate-900 dark:text-slate-100 uppercase">
                          {inv.invoice_number}
                        </td>
                        <td className="px-4 py-3 truncate max-w-37.5 uppercase font-black">
                        ...
                        <td className="px-4 py-3 text-right">
                          ₹{Number(inv.tax_total).toLocaleString("en-IN")}
                        </td>
                        <td className="px-4 py-3 text-right text-slate-900 dark:text-slate-100 font-black">
            ...
            {/* Purchase Register */}
            <div className="space-y-6">
              <h4 className="text-xs font-black text-slate-900 dark:text-slate-100 uppercase tracking-[0.3em] pl-2 border-l-4 border-amber-600">
                Purchase Register (GSTR-2 Data)
              </h4>
              <div className="overflow-x-auto">
                <table className="w-full text-[10px]">
                  <thead className="bg-slate-50 dark:bg-slate-800/50 font-black uppercase tracking-widest text-slate-400 border-y border-slate-100 dark:border-slate-800">
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800 font-bold text-slate-600 dark:text-slate-400">
                    ...
                    {auditData.purchases.list.map((pur: any) => (
                      <tr key={pur.id}>
                        ...
                        <td className="px-4 py-3 truncate max-w-37.5 uppercase font-black">
                          {pur.suppliers?.name || "Unknown"}
                        </td>
                        <td className="px-4 py-3 text-right font-black text-slate-900 dark:text-slate-100">
                          ₹{Number(pur.subtotal).toLocaleString("en-IN")}
                        </td>
                        <td className="px-4 py-3 text-right">
                          ₹{Number(pur.tax_total).toLocaleString("en-IN")}
                        </td>
                        <td className="px-4 py-3 text-right text-slate-900 dark:text-slate-100 font-black">
            ...
            {/* Expense Log */}
            <div className="space-y-6">
              <h4 className="text-xs font-black text-slate-900 dark:text-slate-100 uppercase tracking-[0.3em] pl-2 border-l-4 border-red-600">
            ...
            {auditData.returns.list.length > 0 && (
              <div className="space-y-6">
                <h4 className="text-xs font-black text-slate-900 dark:text-slate-100 uppercase tracking-[0.3em] pl-2 border-l-4 border-slate-600">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800 font-bold text-slate-600 dark:text-slate-400">
                      ...
                      {auditData.returns.list.map((ret: any) => (
                        <tr key={ret.id}>
                          ...
                          <td className="px-4 py-3 text-slate-900 dark:text-slate-100 uppercase font-black italic">
                            {ret.type.replace("_", " ")}
                          </td>
                          <td className="px-4 py-3 truncate max-w-37.5 uppercase font-black">
                            {ret.customers?.name || "Unknown"}
                          </td>
                          <td className="px-4 py-3 text-right text-slate-900 dark:text-slate-100 font-black">
                            ₹{Number(ret.total_amount).toLocaleString("en-IN")}
                          </td>
                        </tr>
                      ))}
                    </tbody>
